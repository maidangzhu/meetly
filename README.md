...
| Voice Ask | “围绕当前内容直接问 AI” | 可先框选文字，再按住 `Fn` 提问；回答面板内继续按 `Fn` 可以带上下文追问 |
...
如果按 Fn 前框选了文字
  -> 将选中文本作为本次 Voice Ask 对话的背景
  -> 用户可以要求翻译、解释、总结、批评或继续讨论
```

Voice Ask 使用通用回答模式，不会因为检测到选区就默认翻译，具体任务仍由用户说出的要求决定。选区和问答历史只保留在当前回答面板中，关闭面板后清空；不会自动粘贴、不会提交输入框，也不会写入会议 transcript。`Fn + Space` 会优先识别为 Dictation，不会误触发 Voice Ask；按 `Esc` 可以取消录音、转写或思考过程。
...
- [Voice Dictation Runtime Design](./docs/VOICE_DICTATION_RUNTIME_DESIGN.md)
